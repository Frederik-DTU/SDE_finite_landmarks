...
epsilon=1
SigmaT = epsilon**2*jnp.eye(3)
LT = jnp.hstack((jnp.eye(3), jnp.zeros((3,3))))

land = landmark_models(k, grad_k)

q0 = jnp.array([-0.5, 0.0, 0.1]).reshape(-1,1)
p0 = jnp.array([-0.1, 0.0, 0.1]).reshape(-1,1)
qT = jnp.array([-0.5, 0.2, 1.0]).reshape(-1,1)
gamma = jnp.array([0.1/jnp.sqrt(1),0.1/jnp.sqrt(2),0.1/jnp.sqrt(3)])
lambda_ = 0.5
x0 = jnp.vstack((q0, p0))

b_fun, sigma_fun = land.get_ms_model(gamma, dim = list(x0.shape), lambda_=lambda_)
betatilde_fun, Btilde_fun, sigmatilde = land.get_ms_approx_model(gamma, list(x0.shape), qT, lambda_)

#t, sim = sde.sim_sde(x0.reshape(-1), b_fun, sigma_fun, n_sim=1)

btilde_fun = lambda t,x,theta=None: betatilde_fun(t,theta)+Btilde_fun(t,theta).dot(x)
sigmatilde_fun = lambda t,x,theta=None: sigmatilde(t,theta)

#t, sim = sde.sim_sde(x0.reshape(-1), btilde_fun, sigmatilde_fun, n_sim=1)
