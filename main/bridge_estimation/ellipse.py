...
q0 = (jnp.vstack((a*jnp.cos(phi), b*jnp.sin(phi))).T).reshape(-1)


a = 1.0
b = 2.0
phi = jnp.linspace(0, 2*jnp.pi, n, endpoint=False)
qT = (jnp.vstack((a*jnp.cos(phi), b*jnp.sin(phi))).T).reshape(-1)
...
def parse_args():
    ...
    # File-paths
    parser.add_argument('--save_path', default='ellipse_models/', 
                        type=str)
